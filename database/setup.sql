-- ============================================================
-- Smart Parking Management System - Full Setup Script
-- Database: smartParkingSystemFinal
-- ============================================================
...
-- Users table: userID is the only determinant for all attributes
CREATE TABLE users (
    userID    INT IDENTITY(1,1) PRIMARY KEY,
    name      VARCHAR(50)  NOT NULL,
    email     VARCHAR(100) NOT NULL UNIQUE,
    password  VARCHAR(64)  NOT NULL,          -- SHA2_256 hex string (64 chars)
    role      VARCHAR(10)  NOT NULL DEFAULT 'user' CHECK (role IN ('admin','user')),
    createdAt DATETIME     NOT NULL DEFAULT GETDATE()
);
GO

-- Vehicles: vehicleID is sole determinant; ownerName kept for display, userID is FK
...
-- ParkingSlots: slotID is sole determinant
...
-- ParkingRates: rateID is sole determinant; vehicleType is also a candidate key
CREATE TABLE rates (
...
-- ParkingRecords: recordID sole determinant; fee computed at checkout
...
-- Payments: paymentID sole determinant; can link to record, reservation, or fine
CREATE TABLE payments (
    paymentID     INT IDENTITY(1,1) PRIMARY KEY,
    recordID      INT           NULL FOREIGN KEY REFERENCES parking_records(recordID),
    reservationID INT           NULL,   -- FK added after reservations table
    fineID        INT           NULL,   -- FK added after fines table
...
-- Reservations: reservationID sole determinant; userID for scoped queries
...
-- Add FK from payments to reservations now that reservations exists
ALTER TABLE payments
    ADD CONSTRAINT FK_payments_reservations
    FOREIGN KEY (reservationID) REFERENCES reservations(reservationID);
GO

-- Fines: fineID sole determinant; can link via recordID or directly via userID/vehicleID
CREATE TABLE fines (
...
-- Add FK from payments to fines now that fines exists
ALTER TABLE payments
    ADD CONSTRAINT FK_payments_fines
    FOREIGN KEY (fineID) REFERENCES fines(fineID);
GO

-- DynamicPricing: separate table for time/demand-based pricing (BCNF — all attributes depend on full composite key)
...
-- Analytics: daily aggregated stats (separate concern, not derivable from a single table)
...
-- ============================================================
-- SAMPLE DATA (passwords hashed with HASHBYTES SHA2_256)
-- Password for users 1-3: 'pass'    → login: pass
-- Password for admin:      'admin'   → login: admin
-- ============================================================
...
-- Trigger 1: Auto-set slot to Occupied when a parking record is inserted
...
-- Trigger 2: Auto-set slot to Available when exitTime is set
...
-- Trigger 3: Auto-set slot to Reserved when reservation is created
...
-- Trigger 4: Auto-free slot when reservation is cancelled or completed
...
-- Trigger 5: Auto-update analytics totals when a payment is marked Paid
...
-- Trigger 6: Prevent double-booking a slot (guard at DB level)
...
-- SP 1: Register a new user (used by backend signup)
CREATE OR ALTER PROCEDURE sp_RegisterUser
    @name     VARCHAR(50),
    @email    VARCHAR(100),
    @password VARCHAR(255),   -- plain text, hashed inside SP
...
-- SP 2: Validate login credentials (returns user row if valid)
...
-- SP 3: Vehicle entry — find available slot and create record (transactional)
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            -- trg_AfterInsertParkingRecord fires automatically
...
-- SP 4: Generate bill and process exit (uses dynamic pricing, transactional)
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            -- trg_AfterUpdateExitTime fires automatically (frees slot)
...
-- SP 5: Create reservation with payment (transactional)
...
-- SP 6: Generate daily analytics report
...
-- View 1: Active vehicles currently in parking (for dashboard)
...
-- View 2: Slot occupancy summary per floor (for floor map)
...
-- View 3: Complete payment history with user and vehicle info
...
-- View 4: Revenue summary by vehicle type
...
-- View 5: Users with their vehicle count and payment totals
...
-- (CRUD, Joins, Grouping, Subqueries, Set Operations)
-- ============================================================

-- Available slots
SELECT * FROM parking_slots WHERE status = 'Available';

-- Vehicles with owner details
...
-- Vehicle types in system vs in rate table (Set operations)
